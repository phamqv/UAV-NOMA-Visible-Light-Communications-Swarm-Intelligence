...
function [lb, ub, dim, fobj] = obj_function_TDMA( sim_para, coordinate_GUs )
    % return the outputs
...
%     B = triu(A,1)';
    ...
    function objf = get_objective_function(PaP)
        ...
        % penalty = penalty + muy*sum(H3.*((sim_para.r_req - Rate).^2));
        penalty = muy*sum(H3.*((sim_para.r_req - Rate).^2));
